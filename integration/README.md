...
- `TestCashLifecycle_MintReceiveRedeem` — mints a real cash token to a
  freshly generated local identity via a live cash_hub, `cashctl receive
  --verify`s it, and `cashctl redeem`s it into a real invoice from the same
  hub. The full mint → receive → redeem round trip, proven live.
- `TestCashInspect_DecodeAndListRecipients` — mints a bearer token, then
  exercises `cashctl cash decode` (local-only) and `cashctl cash
